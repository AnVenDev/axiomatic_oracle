...
export async function verifyTx(opts: {
  ...
  network?: Network;
  indexerUrl?: string;
  maxSkewPastSec?: number;   // default 600
  maxSkewFutureSec?: number; // default 120
  ...
  if (!res.ok) return { verified: false, reason: "tx_not_found", mode: "unknown" };
  const data = await res.json();

  const tx = data.transaction ?? data;
  const noteB64 = tx?.note;
  if (!noteB64) return { verified: false, reason: "note_missing", mode: "unknown" };
