...
def test_get_pipeline_with_real_model(tmp_path, monkeypatch):
    ...
    monkeypatch.setitem(
        model_registry.MODEL_REGISTRY, "property", {"value_regressor": model_file.name}
    )
...
@patch(
    ...
    read_data='{"model_version": "v1", "metrics": {"r2": 0.95}}',
)
@patch(
    "scripts.model_registry._metadata_path_for",
    return_value=Path("models/property/value_regressor_v1_meta.json"),
)
...
def test_model_exists_true():
    with patch(
        "scripts.model_registry._resolve_path",
        return_value=Path("models/property/value_regressor_v1.joblib"),
    ):
...
def test_model_exists_false():
    with patch(
        "scripts.model_registry._resolve_path",
        side_effect=model_registry.ModelNotFoundError(),
    ):
...
@patch(
    "scripts.model_registry.latest_version_filename",
    return_value="value_regressor_v1.joblib",
)
def test_resolve_path_with_fallback(mock_latest, tmp_path, monkeypatch):
    ...
    resolved = model_registry._resolve_path(
        "property", "value_regressor", fallback_latest=True
    )
...
async def test_remote_model_registry_methods():
    ...
    available = await registry.check_remote_availability(
        "property", "value_regressor", "v1"
    )
...
@patch(
    "scripts.model_registry.get_model_metadata", return_value={"metrics": {"r2": 0.85}}
)
@patch(
    "scripts.model_registry._resolve_path",
    return_value=Path("models/property/value_regressor_v1.joblib"),
