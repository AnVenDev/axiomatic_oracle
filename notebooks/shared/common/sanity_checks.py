...
"""
Modulo unificato per:
- Validazione dataset (schema, range, logica, domini, consistenza, temporale)
- Validazione schema-only
- Validazione e normalizzazione di un singolo record (property)
- Benchmark e coerenza prezzi (mediane per location/zone, controlli d'ordine)
"""
...
from notebooks.shared.common.constants import (
    ...
    # score columns (se disponibili)
    CONDITION_SCORE, RISK_SCORE, LUXURY_SCORE, ENV_SCORE,
    # binary columns (se disponibili)
...
__all__ = [
    # Dataset-level (ex data_validation)
    "DataValidator",
    "validate_dataset",
    "validate_schema",
    "validate_property",
...
def validate_columns(df: pd.DataFrame, required_columns: List[str]) -> bool:
    """Verifica che tutte le colonne richieste siano presenti nel DataFrame."""
    missing = [col for col in required_columns if col not in df.columns]
    if missing:
        logger.error("Colonne mancanti: %s", missing)
        return False
    logger.debug("Tutte le colonne richieste sono presenti.")
    return True


def validate_allowed_values(df: pd.DataFrame, allowed_values: Dict[str, List[Any]]) -> bool:
    """Verifica che i valori nelle colonne siano all’interno degli insiemi consentiti."""
    valid = True
    for col, values in allowed_values.items():
        if col in df.columns:
            invalid_values = set(df[col].dropna()) - set(values)
            if invalid_values:
                logger.warning("Valori non ammessi in %s: %s", col, invalid_values)
                valid = False
    return valid


def validate_nulls(df: pd.DataFrame, allow_nulls: Dict[str, bool]) -> bool:
    """Controlla la presenza di valori nulli secondo le regole specificate."""
    valid = True
    for col, allow in allow_nulls.items():
        if col in df.columns:
            null_count = df[col].isna().sum()
            if not allow and null_count > 0:
                logger.error("Colonna %s ha %d valori nulli non consentiti", col, null_count)
                valid = False
    return valid

# Domini locali (record-level)
...
class DataValidator:
    """Validazione completa del DataFrame secondo schema e regole di business."""
    ...
    def _check_ranges(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for col in _SCORE_COLS:
            if col in df:
                ...
                out[col] = {
                    "invalid_count": count,
                    "invalid_pct": float(count / len(df) * 100) if len(df) else 0.0,
                }
                if count:
                    logger.warning("Values out of [0,1] per %s: %d", col, count)
        # Size bounds (euristiche)
        if SIZE_M2 in df:
            ...
            out[SIZE_M2] = {
                ...
                "too_small_pct": float(small / len(df) * 100) if len(df) else 0.0,
                "too_large_pct": float(large / len(df) * 100) if len(df) else 0.0,
            }
            if small or large:
    ...
    def _check_logical(self, df: pd.DataFrame) -> Dict[str, Any]:
        """
        Controlli logici: ID univoci, metriche positive, coerenza piano/piani.
        (Fix del bug: calcoli booleani basati su .all() e non su Series dirette)
        """
        out: Dict[str, Any] = {}
        ...
        if ASSET_ID in df:
            ...
            if not is_unique:
                logger.error("[VALIDATION] Asset IDs non univoci rilevati")
        ...
        if not positive_size:
            logger.warning("[VALIDATION] Alcune dimensioni <= 0 rilevate")
        if not positive_value:
            logger.warning("[VALIDATION] Alcune valutazioni <= 0 rilevate")
        ...
        if {FLOOR, BUILDING_FLOORS}.issubset(df.columns):
            ...
            out["floor_logic"] = ok
            if not ok:
                logger.warning("[VALIDATION] Incoerenza floor/building_floors")

        return out

    def _check_domains(self, df: pd.DataFrame) -> Dict[str, Any]:
        """Controlla (best-effort) domini semantici sulle colonne note, se presenti."""
        out: Dict[str, Any] = {}
        ...
        if ENERGY_CLASS in df:
            ...
            if len(invalid) > 0:
                logger.warning("[VALIDATION] Energy class fuori dominio: %d", len(invalid))
        ...
        for col in (LAST_VERIFIED_TS, PREDICTION_TS):
            if col in df:
                ...
                out[col] = {
                    ...
                    "invalid_pct": float(invalid / len(df) * 100) if len(df) else 0.0,
                }
        if LAG_HOURS in df:
            neg = int((pd.to_numeric(df[LAG_HOURS], errors="coerce") < 0).sum())
            out[LAG_HOURS] = {
                "negative_count": neg,
                "negative_pct": float(neg / len(df) * 100) if len(df) else 0.0,
            }
        return out
...
def validate_dataset(
    ...
    """Wrapper per validazione dataset da usare nei notebook/pipeline."""
...
def validate_schema(df: pd.DataFrame, asset_type: str) -> None:
    """
    Verifica che tutte le colonne richieste dallo schema siano presenti.
    Solleva AssertionError in caso di campi mancanti.
    """
...
def validate_property(prop_data: Dict[str, Any]) -> Dict[str, Any]:
    """
    Validazione e correzione di un singolo asset (property).
    """
    original = dict(prop_data)  # snapshot per diff logging
    errors: List[str] = []
    flags: List[str] = []

    # 0) Normalizza eventuali chiavi legacy/italiane
    _normalize_legacy_keys(prop_data)

    # 1) Coerenza strutturale piani
    ...
    # 3) Minimi ragionevoli per grandezze principali
    ...
    # 4) Valuation/price fallback coerenti
    ...
    if valuation_k < 10.0:
        ...
        fallback_price = (size * 500.0) / 1000.0  # 500 €/m² euristica
    ...
    if errors:
        ...
        logger.warning(
            "[VALIDATION] Asset %s corretto. Errors=%s Flags=%s Changes=%s",
...
def _normalize_legacy_keys(prop_data: Dict[str, Any]) -> None:
    """Mappa chiavi italiane/legacy verso quelle canoniche (best-effort, in-place)."""
...
def price_benchmark(df: pd.DataFrame) -> Tuple[pd.Series, pd.DataFrame | None]:
    """
    Calcola mediane prezzi per location e opzionalmente per zone.
    Returns:
        (city_medians, zone_pivot_or_None)
    """
    if PRICE_COL not in df.columns:
        raise ValueError(f"Manca la colonna '{PRICE_COL}'.")
    # dropna per evitare warning/NaN nella mediana
    city_med = (
...
def critical_city_order_check(
    ...
    """
    Controlla gap significativi tra città adiacenti nella classifica prezzi.
    """
